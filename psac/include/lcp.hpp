...
void lcp_from_sa(const std::string& S, const std::vector<index_t>& SA, const std::vector<index_t>& ISA, std::vector<index_t>& LCP) {
    ...
    for (std::size_t i = 0; i < S.size(); ++i) {
        ...
        while (i+k < S.size() && ISA[i] > static_cast<index_t>(0) && SA[ISA[i]-static_cast<index_t>(1)]+static_cast<index_t>(k) < static_cast<index_t>(S.size()) && S[i+k] == S[SA[ISA[i]- static_cast<index_t>(1)]+static_cast<index_t>(k)])
            k++;
        LCP[ISA[i]] = k;
        h = k;
    }
