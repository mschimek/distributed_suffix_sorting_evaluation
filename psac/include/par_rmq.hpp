...
void bulk_rmq(const std::size_t n, const std::vector<index_t>& local_els,
    ...
    local_sorter.local_sort(ranges, [](const std::tuple<index_t, index_t, index_t>& x,
    ...
    //    return std::get<0>(x) < std::get<0>(y);
    //});
    local_sorter.local_sort(ranges_right, [](const std::tuple<index_t, index_t, index_t>& x,
                 const std::tuple<index_t, index_t, index_t>& y) {
        return std::get<0>(x) < std::get<0>(y);
    });
...
void bulk_rmq_v2(const std::size_t n, const std::vector<index_t>& local_els,
    ...
    for (size_t i = 0; i < ranges.size(); ++i) {
        ...
        int pr = part.rank_of(r-static_cast<index_t>(1));
