// copied from:
// https://github.com/kurpicz/dsss/blob/master/dsss/util/uint_types.hpp
...
class UIntPair {
 ...
 public:
  ...
  UIntPair& operator|=(const unsigned char& b) {
    low_ |= Low(b);
    return *this;
  }

  UIntPair& operator<<=(const int& l) {
    assert(l > 0);
    assert(l <= int(low_bits));
    assert(l <= int(high_bits));
    if (l == high_bits) {
      high_ = 0;
    }
    uint64_t overlap = low_ >> (low_bits - l);
    low_ <<= l;
    high_ <<= l;
    high_ |= High(overlap);
    return *this;
  ...
  // datatype function for mxx, call kamping mapper
  static MPI_Datatype datatype() { return kamping::mpi_datatype<UIntPair>(); }
...
template <>
struct mxx::datatype_builder<dsss::uint40> {
  static MPI_Datatype get_type() { return dsss::uint40::datatype(); }
  static size_t num_basic_elements() { return 1; }
};

template <>
class mxx::has_builder<dsss::uint40> : public std::true_type {};

template <>
struct mxx::datatype_builder<dsss::uint48> {
  static MPI_Datatype get_type() { return dsss::uint48::datatype(); }
  static size_t num_basic_elements() { return 1; }
};

template <>
class mxx::has_builder<dsss::uint48> : public std::true_type {};
