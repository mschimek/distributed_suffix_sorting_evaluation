...
// Note: changed char_type from int to int64_t to handle large alphabets that occur in PDCX
                                      ...
                                      size_t memory)
{
...

    {
        ...
        std::swap(*a, *pm);
        int64_t pivot = ss.get_char(*a, depth);
        pa = pb = a + 1;
        pc = pd = a + n - 1;
        for (;;)
        {
            while (pb <= pc &&
                   (r = static_cast<int64_t>(ss.get_char(*pb, depth)) - pivot) <= 0)
            {
                if (r == 0)
                    std::swap(*pa++, *pb);
                pb++;
            ...
            while (pb <= pc &&
                   (r = static_cast<int64_t>(ss.get_char(*pc, depth)) - pivot) >= 0)
            {
                if (r == 0)
                    std::swap(*pc, *pd--);
                pc--;
