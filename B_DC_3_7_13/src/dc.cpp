...
static inline double running_time = 0;
...
unsigned int getmemusage() {
  ...
  //    return usage.vsize;
  return helper::get_max_mem_bytes();
}
...
class pDCX {
 public:
  ...
  // static const unsigned int DCH[X] = { 3, 6, 6, 5, 6, 5, 4 };
  // // additional chars in tuple static const unsigned int DCD[X] = { 0, 0, 1,
  // 0, 3, 2, 1 };	// depth to sort chars before using first rank
...
#define DISP(obj, attr) ((char*)&obj.attr - (char*)&obj)
    ...
    // type: pair of indexes
    {
      ...
      MPI_Datatype typelist[3] = {getMpiDatatype(p.name),
                                  getMpiDatatype(p.index),
                                  getMpiDatatype(p.unique)};
      int blocklen[3] = {1, 1, 1};
      MPI_Aint displace[3] = {DISP(p, name), DISP(p, index), DISP(p, unique)};

      // MPI_Type_struct(3, blocklen, displace, typelist, &MPI_PAIR);
      MPI_PAIR = kamping::mpi_type_traits<Pair>::data_type();
    ...
    // type: triple of 2 indexes + 1 char
    {
      ...
      MPI_Datatype typelist[3] = {getMpiDatatype(t.rank1),
                                  getMpiDatatype(t.rank2),
                                  getMpiDatatype(t.char1)};
      int blocklen[3] = {1, 1, 1};
      MPI_Aint displace[3] = {DISP(t, rank1), DISP(t, rank2), DISP(t, char1)};

      // MPI_Type_struct(3, blocklen, displace, typelist, &MPI_TRIPLE);
      MPI_TRIPLE = kamping::mpi_type_traits<Triple>::data_type();
    ...
    // type: tuple with X chars and index
    {
      ...
      MPI_Datatype typelist[2] = {getMpiDatatype(t.chars[0]),
                                  getMpiDatatype(t.index)};
      int blocklen[2] = {X, 1};
      MPI_Aint displace[2] = {DISP(t, chars), DISP(t, index)};

      // MPI_Type_struct(2, blocklen, displace, typelist, &MPI_TUPLE_SAMPLE);

      MPI_TUPLE_SAMPLE = kamping::mpi_type_traits<TupleS>::data_type();
    ...
    // type: tuple with X-1 chars, D ranks and index
    {
      ...
      MPI_Datatype typelist[3] = {getMpiDatatype(t.chars[0]),
                                  getMpiDatatype(t.ranks[0]),
                                  getMpiDatatype(t.index)};
      ...
      MPI_TUPLE_NONSAMPLE = kamping::mpi_type_traits<TupleN>::data_type();
...
#undef DISP
  ...
  ~pDCX() { deinit_mpi_datatypes(); }

  template <typename Type>
  void gather_vector(const std::vector<Type>& v, std::vector<Type>& out,
                     unsigned int removelap = 0,
                     MPI_Datatype mdt = GET_MPI_DATATYPE(Type)) {
    ...
    MPI_Gatherv((Type*)v.data(), size, mdt, out.data(), recvcnt, recvoff, mdt,
                ROOT, MPI_COMM_WORLD);
  }
  ...
  // functions for rebalancing the input
  inline uint Extra(int i) { return (i != nprocs - 1) ? (X - 1) : 0; }
  ...
  bool dcx(std::vector<alphabet_type>& input, uint depth, uint K) {
    ...
    MPI_Allgather(&localSize, 1, MPI_UNSIGNED, localSizes, 1, MPI_UNSIGNED,
                  MPI_COMM_WORLD);
    ...
    uint localStride =
        (globalSize + nprocs - 1) / nprocs;  // divide by processors rounding up
    ...
    if (debug) {
      std::cout << "******************** DCX (process " << myproc << ") depth "
                << depth << " ********************" << std::endl;
...

    {
      ...
      for (int i = 1; i < nprocs; ++i) {
        if (debug_rebalance) {
          std::cout << "range sent " << myproc << " -> " << i << " is "
                    << RangeFix(i * localStride, localSizes[myproc],
                                input.size())
                    << " - "
                    << RangeFix((i + 1) * localStride + Extra(i),
                                localSizes[myproc], input.size())
                    << "\n";
        }
      ...
      for (int i = 1; i < nprocs; ++i) {
        if (debug_rebalance) {
          std::cout << "range recv " << i << " -> " << myproc << " is "
                    << RangeFix(localSizes[i], myproc * localStride,
                                localSizeExtra)
                    << "\n"
                    << RangeFix(localSizes[i + 1], myproc * localStride,
                                localSizeExtra)
                    << "\n";
        }
      ...
      MPI_Alltoallv(input.data(), sendcnt, sendoff,
                    GET_MPI_DATATYPE(alphabet_type), recvbuf.data(), recvcnt,
                    recvoff, GET_MPI_DATATYPE(alphabet_type), MPI_COMM_WORLD);
    ...
    // {{{ Sample sort of array R
    {
      ...
      MPI_Gather(samplebuf, samplesize, MPI_TUPLE_SAMPLE, samplebufall,
                 samplesize, MPI_TUPLE_SAMPLE, ROOT, MPI_COMM_WORLD);
      ...
      if (myproc == ROOT) {
        ...
        DBG_ARRAY2(debug_sortsample, "Sample splitters", samplebufall,
                   nprocs * samplesize);
      ...
      DBG_ARRAY2(debug_sortsample, "Splitters positions", splitterpos,
                 nprocs + 1);
      ...
      MPI_Alltoallv(R.data(), sendcnt, sendoff, MPI_TUPLE_SAMPLE,
                    recvbuf.data(), recvcnt, recvoff, MPI_TUPLE_SAMPLE,
                    MPI_COMM_WORLD);
...

    {
      ...
      TupleS temp;  // get last tuple from previous process as basis for name
                    // comparison (cyclicly)
      MPI_Sendrecv(&(R[R.size() - 1]), 1, MPI_TUPLE_SAMPLE,
                   (myproc + 1) % nprocs, MSGTAG, &temp, 1, MPI_TUPLE_SAMPLE,
                   (myproc - 1 + nprocs) % nprocs, MSGTAG, MPI_COMM_WORLD,
                   &status);
      ...
      if (myproc == nprocs - 1) lastname = P.back().name;
    ...
    if (recursion) {
      ...
      uint namesLocalStride =
          (namesGlobalSize + nprocs - 1) / nprocs;  // rounded up
      namesLocalStride +=
      ...
      if (namesGlobalSize > 2 * X * nprocs) {
        ...
        MPI_Alltoallv(P.data(), sendcnt, sendoff, MPI_PAIR, recvBufPair.data(),
                      recvcnt, recvoff, MPI_PAIR, MPI_COMM_WORLD);
        ...
        if (uniqueseq > recvBufPair.size() / 2 && 0) {
          ...
          for (unsigned int i = 0; i < recvBufPair.size(); ++i) {
            ...
            if (helper::is_root()) {
              std::cout << "dup/firstunique name: " << namearray[j]
                        << " - index " << indexarray[j] << "\n";
            }
            ++j;
          }
          ...
          if (helper::is_root()) {
            std::cout << "SAlocal: " << rdcx.localSA.size() << " - indexes "
                      << j << "\n";
            std::cout << "SAlocal: " << rdcx.localSA.size() << " - indexes "
                      << j << "\n";
            std::cout << "SAlocal: " << rdcx.localSA.size() << " - indexes "
          ...
          MPI_Allgather(&SAsize, 1, MPI_UNSIGNED, allSAsize, 1, MPI_UNSIGNED,
                        MPI_COMM_WORLD);
...

    {
      ...
      MPI_Sendrecv(recvBufPair.data(), D, MPI_PAIR,
                   (myproc - 1 + nprocs) % nprocs, MSGTAG, &temp, D, MPI_PAIR,
                   (myproc + 1) % nprocs, MSGTAG, MPI_COMM_WORLD, &status);

      DBG_ARRAY2(debug_recursion, "Pairs P (extra tuples)", temp, D);

      if (myproc ==
          nprocs - 1)  // last processor gets sentinel tuples with lowest ranks
      {
        for (unsigned int i = 0; i < D; ++i) {
    ...
    // *** Sample sort tuple arrays
    {
      ...
      MPI_Gather(samplebuf, X * samplesize, MPI_TUPLE_NONSAMPLE, samplebufall,
                 X * samplesize, MPI_TUPLE_NONSAMPLE, ROOT, MPI_COMM_WORLD);
      ...
      if (myproc == ROOT) {
        std::sort(samplebufall, samplebufall + nprocs * X * samplesize,
                  cmpTupleNCompare);

        DBG_ARRAY2(debug_finalsort, "Sample splitters", samplebufall,
                   nprocs * X * samplesize);
      ...
      for (unsigned int k = 0; k < X; ++k) {
        ...
        MPI_Alltoall(sendcnt[k], 1, MPI_INT, recvcnt[k], 1, MPI_INT,
                     MPI_COMM_WORLD);
      }
      ...
      for (unsigned int k = 0; k < X; ++k) {
        ...
        MPI_Alltoallv(S[k].data(), sendcnt[k], sendoff[k], MPI_TUPLE_NONSAMPLE,
                      recvbuf.data(), recvcnt[k], recvoff[k],
                      MPI_TUPLE_NONSAMPLE, MPI_COMM_WORLD);
    ...
    if (debug) {
      std::cout << "******************** finished DCX (process " << myproc
  ...
  bool run(const char* filename, unsigned int size) {
    // **********************************************************************
    ...
    if (myproc == ROOT) {
      ...
      std::uint64_t file_size = infile.tellg();
      std::cout << "global size: " << globalSize << " requested size: " << size << std::endl;

      char* endptr = NULL;
      unsigned int reducesize = size;
      //    getenv("SIZE") ? strtoul(getenv("SIZE"), &endptr, 10) : 0;
      // if (!endptr || *endptr != '\0') reducesize = 0;

      if (reducesize && file_size > reducesize) globalSize = reducesize;
      if (file_size <= reducesize) {
	      std::cout << "file size smaller than requested size" << std::endl;
	      std::terminate();
      }
      std::cout << "actual size: " << globalSize << std::endl;
    }
    ...
    if (myproc == ROOT) {
      ...
      for (int p = 1; p < nprocs; ++p) {
        ...
        uint readsize =
            (p != nprocs - 1) ? localStride : globalSize - (p * localStride);

        if (helper::is_root()) {
          std::cout << "Read for process " << p << " from pos "
                    << p * localStride << " of length " << readsize
                    << std::endl;
        }

        infile.read((char*)localInput.data(), readsize);
        MPI_Send(localInput.data(), readsize, MPI_CHAR, p, MSGTAG,
                 MPI_COMM_WORLD);
      }
    ...
    {
      MPI_Recv(localInput.data(), localStride, MPI_CHAR, ROOT, MSGTAG,
               MPI_COMM_WORLD, &status);

      int recvcnt;
      MPI_Get_count(&status, MPI_CHAR, &recvcnt);

      uint readsize = (myproc != nprocs - 1)
                          ? localStride
                          : globalSize - (myproc * localStride);
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0) {
      helper::running_time = (tend - tstart);
      std::cout << "\t Running time DCX: " << helper::running_time << std::endl;
    }
  ...
  bool writeSA(const char* filename) {
    ...
    MPI_Gather(&localSAsize, 1, MPI_UNSIGNED, allSAsize.data(), 1, MPI_UNSIGNED,
               ROOT, MPI_COMM_WORLD);
    ...
    if (myproc == ROOT) {
      ...
      for (int p = 1; p < nprocs; p++) {
        MPI_Recv(buffer, allSAsize[p], MPI_UNSIGNED, p, MSGTAG, MPI_COMM_WORLD,
                 &status);
      ...
      MPI_Send(localSA.data(), localSA.size(), MPI_UNSIGNED, ROOT, MSGTAG,
               MPI_COMM_WORLD);
    }
  ...
  bool checkSA() {
    if (debug) {
      std::cout << "******************** SAChecker (process " << myproc
                << ") ********************" << std::endl;
    ...
    MPI_Scan(MPI_IN_PLACE, &localSAsize, 1, MPI_UNSIGNED, MPI_SUM,
             MPI_COMM_WORLD);
    ...
    // * Sample sort of array P by (SA[i])
    {
      ...
      MPI_Sendrecv(recvBufPair.data(), 1, MPI_PAIR,
                   (myproc - 1 + nprocs) % nprocs, MSGTAG, &temp, 1, MPI_PAIR,
                   (myproc + 1) % nprocs, MSGTAG, MPI_COMM_WORLD, &status);

      if (myproc == nprocs - 1)  // last processor gets sentinel pair: virtual
                                 // pair of '$' position after string
      {
    ...
    // * Sample sort of array S by (S[].rank1)
    {
      ...
      MPI_Alltoallv(S.data(), sendcnt, sendoff, MPI_TRIPLE, recvBuf.data(),
                    recvcnt, recvoff, MPI_TRIPLE, MPI_COMM_WORLD);
      ...
      MPI_Sendrecv(recvBuf.data(), 1, MPI_TRIPLE,
                   (myproc - 1 + nprocs) % nprocs, MSGTAG, &temp, 1, MPI_TRIPLE,
                   (myproc + 1) % nprocs, MSGTAG, MPI_COMM_WORLD, &status);

      if (myproc == nprocs - 1)  // last processor gets sentinel triple - which
    ...
    for (uint i = 0; !error && i < iend; ++i)  // -1 due to extra pair at end
    {
      if (S[i].char1 > S[i + 1].char1) {
        ...
        std::cout << "Error: suffix array position " << i + myproc * localStride
                  << " ordered incorrectly.\n";
        error = true;
        ...
        if (S[i + 1].rank2 == globalSize) {
          ...
          std::cout << "Error: suffix array position "
                    << i + myproc * localStride << " ordered incorrectly.\n";
          error = true;
        }
  ...
  bool checkSAlocal(const char* filename, unsigned int size) {
    // **********************************************************************
    // * Read input file size
    ...
    uint localSAsize = localSA.size();
    MPI_Gather(&localSAsize, 1, MPI_INT, recvcnt, 1, MPI_INT, ROOT,
               MPI_COMM_WORLD);
    ...
    if (myproc == ROOT) gSA.resize(globalSize);

    MPI_Gatherv(localSA.data(), localSA.size(), MPI_INT, gSA.data(), recvcnt,
                recvoff, MPI_INT, ROOT, MPI_COMM_WORLD);
...
namespace helper {
auto allgather_int64(std::int64_t local_value) {
  int world_size;
  MPI_Comm_size(MPI_COMM_WORLD, &world_size);

  std::vector<std::int64_t> gathered_values(world_size);

  MPI_Allgather(&local_value, 1, MPI_INT64_T, gathered_values.data(), 1,
                MPI_INT64_T, MPI_COMM_WORLD);
  return gathered_values;
}
std::vector<std::int64_t> sum_blocks_of_48(
    const std::vector<std::int64_t>& data) {
  std::vector<std::int64_t> block_sums;
  const std::size_t block_size = 48;

  for (std::size_t i = 0; i < data.size(); i += block_size) {
    std::size_t end = std::min(i + block_size, data.size());
    std::int64_t sum = std::accumulate(data.begin() + i, data.begin() + end,
...
struct Parameters {
  ...
  std::string algorithm = "TimoPDCX";
  std::string input_path = "";
  std::string output_path = "";
  std::string json_output_path = "";
  std::size_t dcx_variant = 3;
  ...
  std::vector<std::pair<std::string, std::string>> config() const {
    ...
    config_vector.emplace_back("external_iteration", std::to_string(iteration));
    config_vector.emplace_back("json_output_path", json_output_path);
    config_vector.emplace_back("dcx_variant", std::to_string(dcx_variant));
    config_vector.emplace_back("algorithm",
                               algorithm + std::to_string(dcx_variant));
...
void DCXRun(int myproc, const char* input, unsigned int size,
            std::string output) {
  double tstart = MPI_Wtime();
  ...
  if (!output.empty()) {
    dcx.writeSA(output.c_str());
  }

  MPI_Barrier(MPI_COMM_WORLD);

  double tend = MPI_Wtime();
  std::int64_t memory_aver_sa_construction = helper::get_max_mem_bytes();
  auto const all_data = helper::allgather_int64(memory_aver_sa_construction);
  auto const agg_data = helper::sum_blocks_of_48(all_data);
  kamping::measurements::counter().add(
      "mem_after_sa_construction", memory_aver_sa_construction,
      {kamping::measurements::GlobalAggregationMode::max,
       kamping::measurements::GlobalAggregationMode::gather});

  if (myproc == 0) {
    std::cerr << "RESULT" << " algo=pDC" << DCParam::X
              << " time=" << (tend - tstart)
              << (getenv("RESULT") ? getenv("RESULT") : "") << std::endl;
    std::cerr << "RESULT Memory:";
    for (std::size_t i = 0; i < all_data.size(); ++i) {
      std::cerr << all_data[i] << " ";
    }
    std::cerr << std::endl;
    std::cerr << "RESULT AGG-Memory:";
    for (std::size_t i = 0; i < agg_data.size(); ++i) {
      std::cerr << agg_data[i] << " ";
    }
    std::cerr << std::endl;
    auto it = std::max_element(agg_data.begin(), agg_data.end());
    std::cerr << "RESULT AGG-Memory-MAX:" << (it != agg_data.end() ? *it : -1)
              << std::endl;
    std::cerr << "RESULT" << " total_time=" << helper::running_time
              << std::endl;
  }

  bool result = dcx.checkSA();
  if (!result) {
    std::cout << "ERROR: Suffix array checker: " << result << "\n";
  }

  // dcx.checkSAlocal(input, size);
}
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (arg == "--textsize" && i + 1 < argc) {
      params.textsize = std::stoul(argv[++i]);  // convert string → size_t
    } else if (arg == "--dcx" && i + 1 < argc) {
      params.dcx_variant = std::stoul(argv[++i]);
    } else if (arg == "--output" && i + 1 < argc) {
      params.output_path = argv[++i];
    } else if (arg == "--iteration" && i + 1 < argc) {
      params.external_iteration = std::stoul(argv[++i]);
    } else if (arg == "--json_output_path" && i + 1 < argc) {
      params.json_output_path = argv[++i];
    } else {
  ...
  if (params.dcx_variant == 3) {
    DCXRun<DC3Param>(myproc, params.input_path.c_str(), params.textsize,
                     params.output_path);
  } else if (params.dcx_variant == 7) {
    DCXRun<DC7Param>(myproc, params.input_path.c_str(), params.textsize,
                     params.output_path);
  } else if (params.dcx_variant == 13) {
    DCXRun<DC13Param>(myproc, params.input_path.c_str(), params.textsize,
                      params.output_path);
  } else {
  ...
  if (myproc == 0) {
    helper::print_as_jsonlist_to_file({sstream_timer.str()},
                              params.json_output_path + "_timer.json");
    helper::print_as_jsonlist_to_file({sstream_counter.str()},
