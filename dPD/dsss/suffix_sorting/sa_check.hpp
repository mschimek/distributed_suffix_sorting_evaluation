...
bool check(std::vector<IndexType>& sa,
  ...
  dsss::mpi::get_sorter_instance().set_sorter(dsss::mpi::AtomicSorters::Ams);
  ...
  struct sa_tuple {
    ...
    bool operator==(sa_tuple const &other) const {
      return std::tie(rank) == std::tie(other.rank);
    }
  ...
  struct rank_triple {
    ...
    bool operator==(rank_triple const &other) const {
      return std::tie(rank1, rank2, chr) == std::tie(other.rank1, other.rank2, other.chr);
    }
    std::string to_string() const {
      std::stringstream str;
      str << "(" << index << ", " << chr << ", " << rank1 << ", " << rank2 << ")";
      return str.str();
    }
  } DSSS_ATTRIBUTE_PACKED;
  ...
  dsss::mpi::sort_ns(sa_tuples, [](const sa_tuple &a, const sa_tuple &b) {
    return a.sa < b.sa;
  });
  ...
  for (size_t i = 0; i < local_size; ++i) {
    rts.emplace_back(
      rank_triple { sa_tuples[i].rank, sa_tuples[i + 1].rank, text[i], offset + i}
    );
  }

  dsss::mpi::sort_ns(rts, [](const rank_triple& a, const rank_triple& b) {
  ...
  for (size_t i = 0; i < local_size - 1; ++i) {
    bool cond = (rts[i] <= rts[i + 1]);
    if(!cond) {
      std::cout << env.rank() << "i " << i << rts[i].to_string() << std::endl;
      std::cout << env.rank() << "i " << i << rts[i+1].to_string() << std::endl;
    }
    is_sorted &= cond;
  }
